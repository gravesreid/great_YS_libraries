...
public:
	...
	inline const YsList <T> *Seek(int n) const;
...
template <class T> inline YsList <T> *YsList <T>::Seek(int n)
{
	const YsList <T> *cThis;
	cThis=this;
...
template <class T> inline const YsList <T> * YsList <T>::Seek(int n) const
{
	if(this==NULL || n<0)
	{
		return NULL;
