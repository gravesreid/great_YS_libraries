...
YSRESULT YsShellVectorFunction::GetVectorOnPolygon(YsVec3 &dir,const YsShell &shl,const YsShellPolygonHandle plHd,const YsVec3 &pos) const
{
	return YSERR;
}

YSRESULT YsShellVectorFunction::GetVectorOnVertex(YsVec3 &dir,const YsShell &shl,const YsShellVertexHandle vtHd) const
{
	return YSERR;
}

YSBOOL YsShellCrawlStopperFunction::IsStopperVertex(const YsShell &shl,YsShellVertexHandle vtHd) const
{
	return YSFALSE;
}

YSBOOL YsShellCrawlStopperFunction::IsStopperEdge(const YsShell &shl,const YsShellVertexHandle edVtHd[2]) const
{
	return YSFALSE;
}

YSBOOL YsShellCrawlStopperFunction::IsStopperPolygon(const YsShell &shl,const YsShellPolygonHandle plHd) const
{
	return YSFALSE;
}
...
YSRESULT YsShellCrawler::Crawl(const YsShell &shl,const double &dist,YSBOOL watch)
{
	...
	while(distRemain>YsTolerance)
	{
		...
		if(watch==YSTRUE)
		{
			printf("Moved %lf %08x %08x\n",moveDist,nextEdVtHd[0],nextEdVtHd[1]);
		}
				...
				if(visitedPlg.FindElement(tst,shl.GetSearchKey(foundPlHd[j]))!=YSOK)
				{
					...
					for(k=0; k<nPlVt; k++)
					{
						...
						else
						{
							...
							visitedEdge.AddElement(2,vtKey,edgeList.GetN()-1);
						}
					}
				}
			}
