...
YsShellVertexHandle YsShellVtxToVtxTable::FindVertex(const YsShell &shl,unsigned searchKey) const
{
...
YSRESULT YsShellVtxToVtxTable::SelfDiagnostic(const YsShell &shl) const
{
	...
	while((vtHd=shl.FindNextVertex(vtHd))!=NULL)
	{
		...
		if(foundVtHd!=vtHd)
		{
			YsPrintf("  Cannot find correct vertex Key=%d  Found=%x  ShouldBe=%x\n",searchKey,foundVtHd,vtHd);
			res=YSERR;
		}
...
YsShellPolygonHandle YsShellPlgToPlgTable::FindPolygon(const YsShell &shl,unsigned searchKey) const
{
...
YSRESULT YsShellPlgToPlgTable::SelfDiagnostic(const YsShell &shl) const
{
	...
	while((plHd=shl.FindNextPolygon(plHd))!=NULL)
	{
		...
		if(foundPlHd!=plHd)
		{
			YsPrintf("  Cannot find correct polygon Key=%d  Found=%x  ShouldBe=%x\n",searchKey,foundPlHd,plHd);
			res=YSERR;
		}
...
static YsListAllocator <YsShellEdgToPlgTablePolygonList> edgeListAllocator(16);

YsShellEdgToPlgTableEdgeList::YsShellEdgToPlgTableEdgeList() : plgGroupList(edgeListAllocator)
{
	nAlloc++;
}
...
YsShellEdgToPlgTable::YsShellEdgToPlgTable(int hashSize) : YsHashTable <YsShellEdgToPlgTableEdgeList *> (hashSize)
{
	head=NULL;
	tail=NULL;
	nEdge=0;
...
YSRESULT YsShellEdgToPlgTable::AddPolygon(const YsShell &shl,YsShellPolygonHandle plHd)
{
	...
	for(i=0; i<nVtHd; i++)
	{
		...
		{
			edgeGroup=new YsShellEdgToPlgTableEdgeList;
			if(edgeGroup==NULL)
			{
...
YsShellPolygonHandle YsShellSearchTable::GetNeighborPolygon(const YsShell &shl,YsShellPolygonHandle plHd,int n) const
{
	int nPlVt;
	const YsShellVertexHandle *plVtHd;
#ifndef YSLOWMEM
...
int YsShellSearchTable::GetNumEdge(const YsShell &shl) const
{
	return edgToPlg.GetNumEdge();
}

YSRESULT YsShellSearchTable::RewindEdgeEnumHandle
	    (const YsShell &shl,YsShellEdgeEnumHandle &handle) const
{
	return edgToPlg.RewindEdgeEnumHandle(handle);
}

YSRESULT YsShellSearchTable::FindNextEdge
	    (const YsShell &shl,YsShellEdgeEnumHandle &handle) const
{
