...
// Implementation //////////////////////////////////////////
char *YsSwordPolygon::ClassName="YsSwordPolygon";

// Implementation //////////////////////////////////////////
char *YsSwordNewVertexLog::ClassName="YsSwordNewVertexLog";
...
//      plg2->  id1...id2
template <class T>
YSRESULT YsSwordSeparate(YsArray <T> &plg1,YsArray <T> &plg2,YsArray <T> &org,int id1,int id2)
{
	int i,n;
	n=org.GetN();
	for(i=0; i<=id1; i++)
...
char *YsSword::ClassName="YsSword";

const YsSword &YsSword::operator=(const YsSword &from)
{
	// Forbidden
...
int YsSword::GetNumVertexOfPolygon(int id) const
{
...
const YsArray <YsVec3> *YsSword::GetPolygon(int i) const
{
...
const YsArray <int> *YsSword::GetVertexIdList(int i) const
{
	YsList <YsSwordPolygon> *seeker;
	seeker=plg->Seek(i);
	if(seeker!=NULL)
...
YSRESULT YsSword::SetInitialPolygon(int np,const YsVec3 p[],const int id[])
{
	if(plg!=NULL)
	{
		plg->DeleteList();
...
YSRESULT YsSword::GetVertexListOfPolygon(YsVec3 vec[],int maxcount,int id) const
{
	const YsArray <YsVec3> *plg;
	plg=GetPolygon(id);
	if(plg!=NULL)
...
YSRESULT YsSword::GetVertexListOfPolygon(const YsVec3 *vec[],int maxcount,int id) const
{
	const YsArray <YsVec3> *plg;
	plg=GetPolygon(id);
	if(plg!=NULL)
...
YSRESULT YsSword::SetInitialPolygon(int np,const YsVec2 p[],int const id[])
{
	int i;
	YsVec3 *p3;
	YSRESULT res;
...
YSRESULT YsSword::GetVertexListOfPolygon(YsVec2 vec[],int maxcount,int id) const
{
	const YsArray <YsVec3> *plg;
	plg=GetPolygon(id);
	if(plg!=NULL)
...
YSRESULT YsSword::Slash(int nv,const YsVec3 v[])
{
	YsPlane pln;
...
YSRESULT YsSword::SlashByOverlappingPolygon3(int nv,const YsVec3 v[],int note)
{
	int i;
...
YSBOOL YsSword::IsFromOverlappingPolygon3(int i) const
{
	int note;
	return IsFromOverlappingPolygon3(i,note);
}

YSBOOL YsSword::IsFromOverlappingPolygon3(int i,int &note) const
{
	YsList <YsSwordPolygon> *seeker;
	seeker=plg->Seek(i);
	if(seeker!=NULL)
...
YSRESULT YsSword::SlashOnePolygon
 (YsList <YsSwordPolygon> *target,const YsPlane &pln,int nv,const YsVec3 v[])
{
	if(target==NULL)
	{
		return YSOK;
